function addAstra(input) {

    let items = [];
    let pattern =  /([|#])(?<name>[A-Za-z\s]+)\1(?<date>\d{2}\/\d{2}\/\d{2})\1(?<calories>\d{1,5})\1/g;
    
    let totalCal = 0;
    let matches = pattern.exec(input);

    while(matches) {
        items.push({item: matches[2], day: matches[3], calorie: matches[4]});
        totalCal += Number(matches[4]);
        matches = pattern.exec(input);
    }
   
    console.log(`You have food to last you for: ${Math.floor(totalCal / 2000)} days!`);
    for (let i = 0; i < items.length; i++) {
        console.log(`Item: ${items[i].item}, Best before: ${items[i].day}, Nutrition: ${items[i].calorie}`);
    }
}

addAstra('#Bread#19/03/21#4000#|Invalid|03/03.20||Apples|08/10/20|200||Carrots|06/08/20|500||Not right|6.8.20|5|');