function schoolRegister(arr) {
    ...
    let sortedGrade = Object.keys(register).sort((a, b) => a - b);

    for (const grade of sortedGrade) {
        let student = register[grade];
        console.log(`${grade} Grade`);
        console.log(`List of students: ${student.map(a => a.name).join(', ')}`);
        console.log(`Average annual grade from last year: ${average(student.map(a => a.score)).toFixed(2)}`);
        console.log(' ');
    }
