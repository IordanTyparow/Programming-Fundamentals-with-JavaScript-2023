class RefurbishedSmartphones {
    ...
    sellSmartphone(model, desiredStorage) {
        ...
        if (!foundPhone) {
            ...
            } else if (Math.abs(foundPhone.price - desiredStorage) <= 128) {
            ...
            return `${foundPhone.model} was sold for ${soldPrice.toFixed(2)}$`;
        }
    }
    ...
    upgradePhones() {
        if (this.availableSmartphones.length === 0) {
            ...
            let availablePhones = this.availableSmartphones.map(phone => `${phone.model} / ${phone.storage} GB / ${phone.condition} condition / ${phone.price}$`).join('\n');
